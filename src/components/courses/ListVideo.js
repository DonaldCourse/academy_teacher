...
function ListVideo({ lessons }) {
    ...
    const getVideoOfLesson = () => {
        CourseServices.getLessonVideo(selectedLesson).then(res => {
            console.log(res);
            if (res.status == 200) {
                setSlides(res.data);
            }
                        ...
                        <CCardBody>
                            ...
                            <CDataTable
                                ...
                                scopedSlots={{
                                    ...
                                    'thumbnail': (item) => {
                                        return <td style={{ paddingLeft: '10px', verticalAlign: 'middle' }}> {item.thumbnailUrl} </td>
                                    },
                                    'video': (item) => {
                                        return <td style={{ paddingLeft: '10px', verticalAlign: 'middle' }}> {item.videoUrl} </td>
                                    },
                                    'actions':
                                        (item, index) => {
